...
const buildAdminSearchConditions = (search) => {
    if (!search)
        return {};
    return {
        [sequelize_1.Op.or]: [
...
const buildAdminQueryConfig = (whereCondition, sortBy, orderBy, limit, offset) => {
    return {
...
const formatAdminCategoriesResponse = (categories) => {
...
const getByAdmin = async (req, res) => {
    ...
    const whereCondition = buildAdminSearchConditions(search);
    const queryConfig = buildAdminQueryConfig(whereCondition, sortBy, orderBy, limit, offset);
    const paginatedData = await models_1.Category.findAndCountAll(queryConfig);
    const formattedCategories = formatAdminCategoriesResponse(paginatedData.rows);
